...
def load_calibration(seq: str, side: str):
    # dir_path = os.path.dirname(__file__)
    # dir_path = dir_path.split(os.sep)[:-2]
    # dir_path = os.path.sep.join(dir_path)
    # filepath = os.path.join(dir_path, "data", seq, "calib.txt")
    filepath = f"/Users/chris/kitti_dataset/calibs/{seq}/calib.txt"
...
def load_gt_poses(seq: str):
    poses = []
    # dir_path = os.path.dirname(__file__)
    # dir_path = dir_path.split(os.sep)[:-2]
    # dir_path = os.path.sep.join(dir_path)
    # filepath = os.path.join(dir_path, "data", seq, "poses.txt")
    filepath = f"/Users/chris/kitti_dataset/poses/{seq}.txt"
    with open(filepath, "r") as f:
...
def load_image_sequence(seq: str, side: str, len=1000) -> List[Mat]:
    # dir_path = os.path.dirname(__file__)
    # dir_path = dir_path.split(os.sep)[:-2]
    # dir_path = os.path.sep.join(dir_path)
    images = []
    if side == "left":
        ...
        # return [cv2.imread(image) for image in image_paths]
    else:
...
def main():
    ...
    # Camera Properties
    _, p_left = load_calibration(seq, "left")
    _, p_right = load_calibration(seq, "right")
    ...
    # Images
    images_left = load_image_sequence(seq, "left", 250)
    images_right = load_image_sequence(seq, "right", 250)
