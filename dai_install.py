...
parser.add_argument(
    "-dr", "--dry_run", action="store_true", help="Print commands without executing."
)
parser.add_argument(
    "--convert",
    nargs="?",
...
if sys.version_info[0] != 3:
    raise RuntimeError(
        "Examples require Python 3 to run (detected: Python {})".format(
            sys.version_info[0]
        )
    )
...
if requireOpenCv and is_pi and sys.version_info[1] not in prebuiltWheelsPythonVersion:
    print(
        "[WARNING] There are no prebuilt wheels for Python 3.{} for OpenCV, building process on this device may be long and unstable".format(
            sys.version_info[1]
        )
    )
...
downloader_cmd = [
    sys.executable,
    f"{examples_dir}/downloader/downloader.py",
    "--all",
    "--cache_dir",
    f"{examples_dir}/downloader/",
    "--num_attempts",
    "5",
    "-o",
    f"{examples_dir}/models",
]
if args.dry_run:
...
if args.convert != convert_default:
    ...
    blobconverter_cmds = [
        [
            ...
            "-o",
            f"{examples_dir}/models",
            *(["-v", args.convert] if args.convert is not None else []),
        ]
...
if requireOpenCv and thisPlatform == "aarch64":
    ...
    if OPENBLAS_CORE_TYPE != "ARMV8":
        ...
        print(
            f"{WARNING}WARNING:{LINE_CL} Need to set OPENBLAS_CORE_TYPE environment variable, otherwise opencv will fail with illegal instruction."
        )
